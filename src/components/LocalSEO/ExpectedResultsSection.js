  ...
  const metricsData = [
    {
      ...
      hoverStyle: "hover:bg-[#121c2c] hover:border-blue-500/30 text-blue-400"
    },
    {
      icon: <Users size={18} />,
      metric: "+184%",
      ...
      hoverStyle: "hover:bg-[#1c241c] hover:border-green-500/30 text-green-400"
    },
    {
      ...
      hoverStyle: "hover:bg-[#281c14] hover:border-orange-500/30 text-orange-400"
    },
    {
      ...
      hoverStyle: "hover:bg-[#24162e] hover:border-purple-500/30 text-purple-400"
    },
    {
      ...
      hoverStyle: "hover:bg-[#282214] hover:border-amber-500/30 text-amber-400"
    }
  ];
  ...
  return (
    ...
    >
      <div className="w-full max-w-7xl mx-auto flex flex-col space-y-14">
        ...
        {/* Upper Typography Header block from image_94576b.png */}
        <div className="flex flex-col items-start space-y-4">
          {/* Micro Status Pill Badge */}
        ...
        {/* Statistical Metrics Grid Framework (Responsive Layout) */}
          ...
          {metricsData.map((data, idx) => (
            <div
              ...
              className={`
                ...
                ${data.hoverStyle.split(' text')[0]}
              `}
              style={{ minHeight: "220px" }}
            >
              {/* Mini Icon Capsule Block */}
              <div 
                className={`w-9 h-9 rounded-xl bg-white/[0.03] border border-white/5 flex items-center justify-center shrink-0 transition-colors duration-300 group-hover:bg-white/10 group-hover:border-white/20 ${data.hoverStyle.split('hover:border-')[1] ? data.hoverStyle.split(' ')[2] : 'text-zinc-400'}`}
              >
                {data.icon}
              </div>

              {/* Statistical Analytics Copy Stacks */}
              <div className="flex flex-col space-y-1.5 w-full">
                {/* Big Dynamic Counter Number */}
                <h3 className="text-3xl md:text-4xl font-black tracking-tight text-white group-hover:text-zinc-50 transition-colors">
                  {data.metric}
                </h3>
                
                {/* Metric Structural Label */}
                <span className="text-sm font-bold text-zinc-300 tracking-tight">
                  {data.label}
                </span>

                {/* Footnote Micro Metadata */}
